(() => {
    ...
    function initSliders() {
        if (document.querySelector(".main-burger__description")) new core(".main-burger__description", {
        ...
        if (document.querySelector("._staro-mixaylovskoe__swiper")) new core("._staro-mixaylovskoe__swiper", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
            ...
            grabCursor: true,
            navigation: {
                prevEl: ".body-staro-mixaylovskoe__button_prev",
                nextEl: ".body-staro-mixaylovskoe__button_next"
            },
            on: {
                init: function(swiper) {
                    const slider = swiper.slidesEl;
                    const project = slider.closest(".project");
                    const counter = project.querySelector(".footer-project__counter");
                    const counterIndex = counter.querySelector("span");
                    counter.innerHTML = `<span>1</span> — ${swiper.slides.length}`;
                    counterIndex.innerHTML = swiper.snapIndex;
                    const roomName = project.querySelector(".footer-project__name");
                    roomName.innerHTML = swiper.slides[swiper.snapIndex].dataset.projectRoom;
                },
                realIndexChange: function(swiper) {
                    ...
                    const counter = project.querySelector(".footer-project__counter");
                    const counterIndex = counter.querySelector("span");
                    counterIndex.innerHTML = swiper.snapIndex + 1;
                    const roomName = project.querySelector(".footer-project__name");
                    roomName.innerHTML = swiper.slides[swiper.snapIndex].dataset.projectRoom;
                }
            }
        });
        if (document.querySelector(".commerc-page__slider")) new core(".commerc-page__slider", {
            modules: [ EffectFade, Autoplay ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
        ...
        if (document.querySelector(".bottom-reviews__body")) new core(".bottom-reviews__body", {
            ...
            grabCursor: true,
            scrollbar: {
                el: ".bottom-reviews__scrollbar",
                draggable: true
            },
            navigation: {
                prevEl: ".bottom-reviews__arrow_prev",
                nextEl: ".bottom-reviews__arrow_next"
            },
            on: {
                init: function(swiper) {
                    const counter = document.querySelector(".bottom-reviews__counter");
                    const counterIndex = counter.querySelector("span");
                    counter.innerHTML = `<span>1</span> — ${swiper.slides.length}`;
                    counterIndex.innerHTML = swiper.snapIndex;
                },
        ...
        if (document.querySelector(".videos-page__body")) new core(".videos-page__body", {
            modules: [ Scrollbar ],
            observer: true,
            observeParents: true,
            slidesPerView: 4,
        ...
        if (document.querySelector(".blueprints-page__slider")) new core(".blueprints-page__slider", {
            modules: [],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
...
*/
    ...
    gsapWithCSS.fromTo(".transition__item_1", {
        x: "0%"
    }, {
        x: "-100%",
        duration: 1
    });
    gsapWithCSS.fromTo(".transition__item_2", {
        x: "0%"
    }, {
        x: "-100%",
        duration: 1,
    ...
    gsapWithCSS.fromTo(".transition__item_3", {
        x: "0%"
    }, {
        x: "-100%",
        duration: 1,
    ...
    if (document.querySelector(".form-leave__button")) document.querySelector(".form-leave__button").addEventListener("click", (() => {
        const formItems = document.querySelectorAll(".form-leave__item");
        formItems.forEach((formItem => {
            const formInput = formItem.querySelector("input");
            formItem.addEventListener("input", (() => {
                if (formItem.querySelector(".form__error")) formItem.querySelector(".form__error").remove();
            }));
            formValidate.validateInput(formInput);
        }));
    }));
    document.querySelectorAll("a").forEach((link => {
        if (!link.classList.contains("_button-link")) link.addEventListener("click", (e => {
            e.preventDefault();
    ...
    if (document.querySelector(".videos-page__play")) {
        const buttons = document.querySelectorAll(".videos-page__play");
        buttons.forEach((button => {
            button.addEventListener("click", (() => {
                const videoItem = button.closest(".videos-page__item");
                const video = videoItem.querySelector(".videos-page__video video");
                video.play();
...
*/
    ...
    let rotate;
    if (window.innerHeight > 1e3 || window.innerWidth > 992) rotate = "-100%"; else if (window.innerHeight > 500) rotate = "-104%"; else [ rotate = "-105%" ];
    if (document.querySelector(".images-page")) {
        const tl = gsapWithCSS.timeline();
        const images = document.querySelectorAll(".images-page__image");
        tl.fromTo(".images-page", {
            y: "20%"
        }, {
            y: `${rotate}`
        });
        ...
        const counter = document.querySelector(".main-page__counter");
        counter.innerHTML = `<span>0</span> — ${images.length}`;
        const counterIndex = counter.querySelector("span");
        const speed = .05;
        let posX = 0, coordXper = 0;
        function setMouseParallaxStyle() {
            const distX = coordXper - posX;
            posX += distX * speed;
            images.forEach((image => {
                const rotate = posX / 5;
                image.style.cssText = `transform: rotate(${rotate}deg)`;
            }));
            requestAnimationFrame(setMouseParallaxStyle);
        }
        setMouseParallaxStyle();
        document.body.addEventListener("mousemove", (e => {
            const parralaxWidth = document.body.offsetWidth;
            const coordX = e.pageX - parralaxWidth / 2;
            coordXper = coordX / parralaxWidth * 100;
        }));
        let lock = true;
        const showSlides = [];
        const observer = new IntersectionObserver((entries => {
            entries.forEach((entry => {
                if (!lock) {
                    const index = entry.target.dataset.room;
                    if (!showSlides.includes(+index)) showSlides.push(+index); else {
                        const foundedIndex = showSlides.findIndex((value => value === +index));
                        showSlides.splice(foundedIndex, 1);
                    }
                    if (showSlides.length === 0) changeCount(0); else changeCount(showSlides[showSlides.length - 1]);
                }
            }));
            lock = false;
        }));
    ...
    class gallerySlider {
        static slider;
        static init() {
            if (document.querySelector(".gallery-page__body")) this.slider = new core(".gallery-page__body", {
                modules: [ Scrollbar ],
                observer: true,
                observeParents: true,
                slidesPerView: 4,
    ...
    gallerySlider.init();
    if (document.querySelector(".footer-gallery__change ")) document.querySelector(".footer-gallery__change ").addEventListener("click", (e => {
        const gallery = e.target.closest(".wrapper");
        if (gallery.classList.contains("list")) {
            gallerySlider.init();
            e.target.innerHTML = "Проекты галереей";
        } else {
            gallerySlider.destroy();
            e.target.innerHTML = "Проекты списком";
        }
    ...
    class aboutSlider {
        static slider;
        static init() {
            if (document.querySelector(".about-page__images")) this.slider = new core(".about-page__images", {
                modules: [ Mousewheel ],
                direction: "vertical",
                observer: true,
                observeParents: true,
                ...
                on: {
                    resize: function(swiper) {
                        if (window.innerWidth < 768) swiper.destroy(true, true);
                    }
                }
            });
        }
    ...
    window.addEventListener("resize", (() => {
        if (window.innerWidth > 768) aboutSlider.init();
    }));
    window.addEventListener("load", (() => {
        if (window.innerWidth < 768) aboutSlider.destroy(); else aboutSlider.init();
    }));
    window["FLS"] = true;
    isWebp();
    menuInit();
